...
public class OSSUtil {
    ...
    @Value("${tomodachi.aliyun.oss.bucket-name}")
    private String bucketName;
    @Value("${tomodachi.aliyun.oss.url-prefix}")
    ...
    public String upload(MultipartFile file, String folder) {
        try {
            ...
            if (filename == null)
                throw new BusinessException(ErrorCode.NULL_ERROR, "上传操作异常");
            ...
            throw new BusinessException(ErrorCode.SYSTEM_ERROR, "上传文件失败, 请稍后再试");
        }
    }
    ...
    public void delete(String url) {
        try {
            if (!url.startsWith(urlPrefix))
                throw new BusinessException(ErrorCode.PARAMS_ERROR, "删除操作异常");
            String filename = url.substring(urlPrefix.length());
            ossClient.deleteObject(bucketName, filename);
        } catch (Exception e) {
            log.error("删除文件失败: {}", e.getMessage(), e);
            throw new BusinessException(ErrorCode.SYSTEM_ERROR, "删除文件失败, 请稍后再试");
        }
    }
}
